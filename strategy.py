...
def build_market_snapshot(df_scan: pd.DataFrame, df_1h: pd.DataFrame) -> Optional[MarketSnapshot]:
    ...
    # Работаем по последней закрытой 5m свече
    close_now = float(df_scan["close"].iloc[-2])
    close_prev = float(df_scan["close"].iloc[-3])

    # Фильтр по последней закрытой 1h свече
...
def update_trade_state_for_bar(trade_state: TradeState, df_scan: pd.DataFrame):
    ...
    # Проверяем стоп/тейк по последней закрытой 5m свече
...
def compute_bar_signal(
    ...
    snapshot = build_market_snapshot(df_scan, df_1h)
    if snapshot is None:
        return None, None
