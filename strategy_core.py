...
def build_market_snapshot(df_scan: pd.DataFrame, df_1h: pd.DataFrame) -> Optional[MarketSnapshot]:
    ...
    if len(df_1h) < max(EMA_LEN, VOL_MA_LEN, IMPULSE_LOOKBACK_H) + 2:
        return None
...
def process_user_symbol_fast(
    ...
    _ = sens  # для совместимости/читаемости
    ...
    long_filter_ok = enable_long and long_trend_ok and snapshot.impulse_ok and snapshot.vol_ok
    short_filter_ok = enable_short and short_trend_ok and snapshot.impulse_ok and snapshot.vol_ok

    raw_buy = None
    if long_filter_ok and long_h is not None and long_l is not None:
        long_reclaim = close_now > long_h and close_prev <= long_h
        if long_reclaim:
    ...
    raw_sell = None
    if short_filter_ok and short_h is not None and short_l is not None:
        short_breakdown = close_now < short_l and close_prev >= short_l
        if short_breakdown:
